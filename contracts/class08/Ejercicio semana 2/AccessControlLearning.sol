...
contract AccessControlLearning {
    ...
    event RenounceOwnership(address admin);
    // event RenounceOwnership

    // 1. definir un mapping doble para guardar datos en una matriz
    // mapping 1 -> address => role
    // mapping 2 -> role => boolean
    mapping(address funcion => mapping(bytes32 nombreRol => bool siOno))
        private roles;
    ...
    function hasTemporaryRole(
        ...
        if (roles[_account][_role]) {
            return (roles[_account][_role], vecesAdmin);
        }
        return (false, 0);
    }
}
